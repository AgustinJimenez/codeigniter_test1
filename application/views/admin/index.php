					...
					<tr>
						...
						<td>
							<?php foreach ($user->groups as $group): ?>
								<?= anchor("admin/edit_group/".$group->id, htmlspecialchars($group->name,ENT_QUOTES,'UTF-8'), 'class="btn btn-xs btn-link btn-flat"') ;?><br />
							<?php endforeach; ?>
						</td>
						<td><?= ($user->active) ? anchor("admin/deactivate/".$user->id, lang('index_active_link'), 'class="btn btn-success btn-flat"') : anchor("admin/activate/". $user->id, lang('index_inactive_link'), 'class="btn btn-warning btn-flat"');?></td>
						
						<td>
							<div class="btn-group">
								<a href="<?= "admin/edit_user/{$user->id}" ?>" class="btn btn-flat btn-primary">
